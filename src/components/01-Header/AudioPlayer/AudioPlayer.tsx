...
export const AudioPlayer: React.FC<AudioPlayerType> = ({
  ...
  return (
    <div className={css.player}>
      <audio muted={muted} ref={audioPlayer} preload="metadata" />
      <div className={css.controls}>
        <div className={css.control} onClick={togglePreviousTrack}>
          <MdSkipPrevious />
        </div>
        <div className={`${css.control} ${css.rewind}`} onClick={backTen}>
          <MdFastRewind />
        </div>

        <div className={css.control} onClick={togglePlay}>
          {isPlaying ? <MdPause /> : <MdPlayArrow />}
        </div>

        <div className={`${css.control} ${css.rewind}`} onClick={forwardTen}>
          <MdFastForward />
        </div>
        <div className={css.control} onClick={toggleNextTrack}>
          <MdSkipNext />
        </div>
        ...
        <div className={css.progressBarBlock}>
          <div className={css.timers}>
            ...
            <div className={css.timer}>
              {duration && !isNaN(duration) && duration < 35640 ? calcTime(duration) : '00 : 00'}
            </div>
          </div>
