...
export const RegistrationForm: React.FC<RegistrationFormType> = ({registrTC}) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, onSubmitProps) => {
            registrTC(values.login, values.email, values.password, values.acceptTerms, onSubmitProps.setStatus, onSubmitProps.setSubmitting)
    ...
    return (
        ...
        <form className={css.loginForm} onSubmit={formik.handleSubmit}>
            <p><b>Attention!</b> It's just a registration form. Unfortunately, the API of this social network does not provide registration endpoints.</p>
            <p>If you want to see a regular user account, please login with: <b>Login:</b> free@samuraijs.com <b>Password:</b> free</p>
            <InputFormik mustFilled label={"login"} placeholder={"login"} getFieldProps={formik.getFieldProps("login")}
                         errors={formik.errors.email} type={"text"}/>
            <InputFormik mustFilled label={"email"} placeholder={"email"} getFieldProps={formik.getFieldProps("email")}
                         errors={formik.errors.email} type={"text"}/>
            <InputFormik mustFilled label={"password"} placeholder={"password"} getFieldProps={formik.getFieldProps("password")}
                         errors={formik.errors.password} type={"password"}/>
            <InputFormik mustFilled label={"confirm password"} placeholder={"confirm password"} getFieldProps={formik.getFieldProps("confirmPassword")}
                         errors={formik.errors.password} type={"password"}/>
            ...
            <div className={css.buttonBlock}>
                <AuthButton disabled={!formik.values.acceptTerms} type="submit">Sign Up</AuthButton>
