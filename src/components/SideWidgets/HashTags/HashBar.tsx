...
export const HashBar = () => {
    ...
    function generateHashtag() {
        if (text.trim() !== '') {
            const strUpperCase = text.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
    ...
    return(
        <nav className={css.hash}>
            ...
            <div >
                {hastags.map((e, index)=>{
                    return <a key={index} className={css.tag}
                              href={`https://www.google.com/search?q=%23${e.tag.slice(1)}`}>{e.tag}</a>
                })}
            </div>
            <div>
            </div>
