...
export const ProfileDataForm: React.FC<ProfileDataFormType> = ({ userProfile, saveProfile, resultCode }) => {
  const formik = useFormik({
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      for (let key in values2.contacts) {
        ...
        // @ts-ignore
        values2.contacts[key] =
          contact.length && contact.slice(0, 8) === 'https://' ? contact : contact.length ? 'https://' + contact : '';
      }
  ...
  return (
    <form className={css.formBlock} onSubmit={formik.handleSubmit}>
      ...
      <InputFormik
        htmlFor={'aboutMe'}
        label={'About Me'}
        getFieldProps={formik.getFieldProps('aboutMe')}
